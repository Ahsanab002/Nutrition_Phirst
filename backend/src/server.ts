...
// ----- CORS ORIGIN SETUP -----
/**
 * Default frontend origins if FRONTEND_URL is not set.
 * You can override these from Railway by setting:
 * FRONTEND_URL=https://nutritionphirst.com,https://www.nutritionphirst.com,http://localhost:5173
 */
const defaultFrontendOrigins = [
  'http://localhost:5173',
  'http://localhost:3000',
  'https://nutritionphirst.com',
  'https://www.nutritionphirst.com',
];
...
app.use(
  cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'x-local-admin'],
  }),
);
...
// ----- IMAGES CORS + STATIC SERVE -----

// Use the same origin rules for images
...
app.use(
  '/images',
  cors({
    origin: imagesOriginChecker as any,
    credentials: true,
    methods: ['GET'],
...
app.get('/test-images', (req, res) => {
  ...
  res.json({
    ...
    sampleFolders: fs.existsSync(imagesDir)
...
app.get('/api/admin-debug', (req, res) => {
  ...
  res.json({
    ...
    headers: {
      'x-local-admin': req.headers['x-local-admin'],
...
// Root route to confirm backend status
app.get('/', (req, res) => {
  res.status(200).json({
    success: true,
    message: 'NeuroPhirst backend is live on Railway 🚀',
    timestamp: new Date().toISOString(),
  });
});
